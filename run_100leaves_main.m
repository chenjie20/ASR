...
%         end        
    ...
    for time_idx = 1 : testing_times
        ...
        dlmwrite('results/leaves100_final_result.txt', [raito, time_idx, alpha, beta, acc, nmi, purity, fmeasure, ri, ari, time_cost, current_sparsity_ratio, iter1, iter2] , '-append', 'delimiter', '\t', 'newline', 'pc');
    end
    
    dlmwrite('results/leaves100_final_average_result.txt', [raito, alpha, beta, mean(final_clustering_accs(raito_idx, :)), std(final_clustering_accs(raito_idx, :)), mean(final_clustering_nmis(raito_idx, :)), std(final_clustering_nmis(raito_idx, :)), ...
...
% save('leaves100_final_result.mat', 'final_clustering_accs', 'final_clustering_nmis', 'final_clustering_purities', 'final_clustering_fmeasures', 'final_clustering_ris', ...
