...
for raito_idx = 1 : length(raitos)
    ...
    for nv_idx = 1 : nv
        if raito < 1
             pos = randperm(total_num);
             num = floor(total_num * raito);
             sample_pos = zeros(1, total_num);
...
%         end
    ...
    for time_idx = 1 : testing_times
        ...
        dlmwrite('results/reuters_final_result.txt', [raito, time_idx, alpha, beta, acc, nmi, purity, fmeasure, ri, ari, time_cost, current_sparsity_ratio, iter1, iter2] , '-append', 'delimiter', '\t', 'newline', 'pc');
    end
    
    dlmwrite('results/reuters_final_average_result.txt', [raito, alpha, beta, mean(final_clustering_accs(raito_idx, :)), std(final_clustering_accs(raito_idx, :)), mean(final_clustering_nmis(raito_idx, :)), std(final_clustering_nmis(raito_idx, :)), ...
...
% save('reuters_final_result.mat', 'final_clustering_accs', 'final_clustering_nmis', 'final_clustering_purities', 'final_clustering_fmeasures', 'final_clustering_ris', ...
