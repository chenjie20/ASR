...
%---------------------- load data -------------------------
load('data/flower17_Kmatrix.mat');
n = length(Y);
nv = size(KH, 3);
K = length(unique(Y));
gnd = Y;
...
%--------------------------------------------------------------------------
alphas = [1e-7, 1e-7, 1e-7, 1e-4];
betas = [1e-4, 1e-4, 1e-4, 1e-3];
...
for raito_idx = 1 : length(raitos)
    ...
    for time_idx = 1 : testing_times
        ...
        dlmwrite('results/flower_final_result.txt', [raito, time_idx, alpha, beta, acc, nmi, purity, fmeasure, ri, ari, time_cost, current_sparsity_ratio, iter1, iter2] , '-append', 'delimiter', '\t', 'newline', 'pc');
    end
    
    dlmwrite('results/flower_final_average_result.txt', [raito, alpha, beta, mean(final_clustering_accs(raito_idx, :)), std(final_clustering_accs(raito_idx, :)), mean(final_clustering_nmis(raito_idx, :)), std(final_clustering_nmis(raito_idx, :)), ...
...
% save('flower_final_result_new4.mat', 'final_clustering_accs', 'final_clustering_nmis', 'final_clustering_purities', 'final_clustering_fmeasures', 'final_clustering_ris', ...
