...
alphas = [1e-8, 1e-8, 1e-06, 1e-4];
betas = [0.1, 0.1, 5e-3, 0.1];
...
alpha_len = length(alphas);
beta_len = length(betas);
raito_len = length(raitos);
max_iters = 500;
...
for raito_idx = 1 : length(raitos)
    ...
    for time_idx = 1 : testing_times
        ...
        dlmwrite('results/caltech101_final_result.txt', [raito, time_idx, alpha, beta, acc, nmi, purity, fmeasure, ri, ari, time_cost, current_sparsity_ratio, iter1, iter2] , '-append', 'delimiter', '\t', 'newline', 'pc');
    end
    
    dlmwrite('results/caltech101_final_average_result.txt', [raito, alpha, beta, mean(final_clustering_accs(raito_idx, :)), std(final_clustering_accs(raito_idx, :)), mean(final_clustering_nmis(raito_idx, :)), std(final_clustering_nmis(raito_idx, :)), ...
...
% save('bbc_final_result.mat', 'final_clustering_accs', 'final_clustering_nmis', 'final_clustering_purities', 'final_clustering_fmeasures', 'final_clustering_ris', ...
